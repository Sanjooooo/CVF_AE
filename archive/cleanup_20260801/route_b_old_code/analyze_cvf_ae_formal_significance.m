...
for sceneId = scenes
    ...
    for idx = 1:numel(algorithms)
        ...
        sceneRows(idx, :) = {cvfAePaperSceneId(sceneId), algorithm, source, numel(cvf), ...
...
for sceneId = scenes
    ...
    for idx = 1:numel(algorithms)
        ...
        [p, h, stats] = ranksum(cvf, baseline, 'alpha', alpha, ...
            'tail', 'both', 'method', 'approximate');
        delta = localLowerIsBetterCliffsDelta(cvf, baseline);
        sceneRows(idx, :) = {cvfAePaperSceneId(sceneId), algorithm, "formal-ablation", ...
...
for sceneIdx = 1:numel(scenes)
    ...
    rows(sceneIdx, :) = {cvfAePaperSceneId(sceneId), numel(allAlgorithms), numel(values), ...
...
fprintf(fid, '- 正式消融：`%s`\n\n', ablationDir);
fprintf(fid, '编号规则：内部第三个正式地图的 ID 为 `4`，所有论文表格和文字统一显示为 Scene 3。\n\n');
